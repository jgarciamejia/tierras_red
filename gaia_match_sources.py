...
# load x and y positions for comp stars in file 
...
# Astrometric solution is required.
if "STDCRMS" not in hdr:
  logging.error("{0:s} has no astrometric solution".format(filename))
  continue

# Check for bad astrometry.
...
matched_flux = flux[ismatched]
matched_fluxerr = fluxerr[ismatched]
matched_pkht = pkht[ismatched]
...
outfile = basefile + "_gaia.txt"

numpy.savetxt(outfile,
              numpy.column_stack((matched_row,
                                  matched_x,
                                  matched_y,
                                  matched_flux / exptime,
                                  matched_fluxerr / exptime,
                                  matched_pkht,
                                  matched_gaia["G"],
                                  matched_gaia["BP"],
                                  matched_gaia["RP"],
                                  rerr)),
              fmt="%-5d %12.6f %12.6f %16.6f %16.6f %5.0f %6.3f %6.3f %6.3f %6.3f")
